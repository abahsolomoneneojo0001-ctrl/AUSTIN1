    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-5xl mx-auto pb-12 relative">
      ...
      {showToast && newAchievements.length > 0 && (
        <div className="fixed top-20 right-4 z-50 animate-in slide-in-from-top-4 fade-in duration-300">
          <div className="bg-ff-surface border border-ff-primary/30 rounded-2xl p-4 shadow-2xl flex items-start gap-4 max-w-sm">
            <div className="w-10 h-10 rounded-full bg-ff-primary/20 flex items-center justify-center shrink-0">
              <Trophy className="w-5 h-5 text-ff-primary" />
            </div>
            <div className="flex-1">
              <h4 className="font-bold text-white mb-1">Achievement Unlocked!</h4>
            ...
            <button onClick={() => setShowToast(false)} className="text-ff-muted hover:text-white transition-colors">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
      ...
      <div className={cn(
        "relative overflow-hidden rounded-[32px] p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-8 border",
        data.streak > 0 
          ? "bg-ff-surface border-ff-secondary/30" 
          : "bg-ff-surface border-ff-surface"
      )}>
        {data.streak > 0 && (
          <div className="absolute -right-20 -top-20 w-64 h-64 bg-ff-secondary/20 rounded-full blur-3xl pointer-events-none" />
        )}
        
        <div className="flex flex-col sm:flex-row items-center text-center sm:text-left gap-6 z-10">
          <div className={cn(
            "w-20 h-20 rounded-full flex items-center justify-center shrink-0 shadow-2xl",
            data.streak > 0 ? "bg-ff-secondary" : "bg-ff-bg border-2 border-dashed border-ff-muted"
          )}>
            <Flame className={cn("w-10 h-10", data.streak > 0 ? "text-white" : "text-ff-muted")} />
          </div>
          <div>
            <h2 className="font-display text-4xl tracking-wide mb-1 text-white">
            ...
            return (
              <div key={i} className="flex flex-col items-center gap-2">
                <div className={cn(
                  ...
                  isCompleted ? "bg-ff-primary text-black" : "bg-ff-bg text-ff-muted border border-ff-surface"
                )}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="p-6 rounded-[24px] bg-ff-surface border border-ff-surface hover:border-ff-primary/30 transition-colors">
              <p className="text-xs text-ff-muted font-bold tracking-wider mb-2">TOTAL WORKOUTS</p>
              <p className="text-5xl font-mono font-bold text-ff-text">{data.stats.totalWorkouts}</p>
            </div>
            <div className="p-6 rounded-[24px] bg-ff-surface border border-ff-surface hover:border-ff-secondary/30 transition-colors">
              <p className="text-xs text-ff-muted font-bold tracking-wider mb-2">CALORIES BURNED</p>
              <p className="text-5xl font-mono font-bold text-ff-text">{data.stats.caloriesBurned.toLocaleString()}</p>
            </div>
          </div>

          {/* Activity Chart */}
          <div className="p-6 rounded-[24px] bg-ff-surface border border-ff-surface">
            ...
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data.chartData} margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
                  <Tooltip 
                    cursor={{ fill: 'var(--color-ff-surface)' }}
                    contentStyle={{ backgroundColor: 'var(--color-ff-bg)', border: '1px solid var(--color-ff-surface)', borderRadius: '12px', color: '#FFFFFF' }}
                  />
            ...
            {ACHIEVEMENTS.map((ach) => {
              ...
              return (
                <div 
                  key={ach.id} 
                  className={cn(
                    "p-6 rounded-[24px] flex flex-col items-center text-center gap-3 relative overflow-hidden group transition-all",
                    isUnlocked 
                      ? `bg-ff-surface border ${ach.border}` 
                      : "bg-ff-surface border border-ff-surface opacity-50 grayscale"
                  )}
                >
                  {isUnlocked && <div className={cn("absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity", ach.bg.replace('/20', '/5'))} />}
                  <div className={cn(
                    "w-16 h-16 rounded-full flex items-center justify-center",
                    isUnlocked ? ach.bg : "bg-ff-bg"
                  )}>
                    <Icon className={cn("w-8 h-8", isUnlocked ? ach.color : "text-ff-muted")} />
                  </div>
                  <div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            <div className="p-5 rounded-[20px] bg-ff-surface border border-ff-surface flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              ...
              <button 
                ...
                className={cn(
                  ...
                  data.connectedApps.appleHealth ? "bg-ff-bg text-ff-text" : "bg-ff-primary text-black hover:bg-ff-primary/80"
                )}
              >
            ...
            <div className="p-5 rounded-[20px] bg-ff-surface border border-ff-surface flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              ...
              <button 
                ...
                className={cn(
                  "px-4 py-2 rounded-xl text-xs font-bold transition-colors w-full sm:w-auto",
                  data.connectedApps.googleFit ? "bg-ff-bg text-ff-text" : "bg-ff-primary text-black hover:bg-ff-primary/80"
                )}
              >
        ...
        <div className="space-y-4 md:col-span-3 mt-4">
          ...
          <div className="bg-ff-surface border border-ff-surface rounded-[24px] overflow-hidden">
            {data.logs && data.logs.length > 0 ? (
              <div className="divide-y divide-ff-bg">
                {[...data.logs].reverse().map((log: any, i: number) => (
                  <div key={log.id || i} className="p-5 flex items-center justify-between hover:bg-ff-bg/50 transition-colors">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-full bg-ff-bg flex items-center justify-center">
                    ...
                    <div className="text-right">
                      <p className="font-mono font-bold text-ff-text">{log.duration}</p>
                      <p className="text-xs text-ff-secondary font-bold">{log.calories} kcal</p>
                    </div>
                  </div>
                ))}
              </div>
