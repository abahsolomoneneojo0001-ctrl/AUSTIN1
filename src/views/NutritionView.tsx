...
export default function NutritionView() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <button 
            ...
            className="flex flex-1 sm:flex-none justify-center items-center gap-2 bg-ff-surface border-2 border-ff-primary text-ff-text px-4 py-2 rounded-full font-bold text-sm hover:bg-ff-bg transition-colors"
          >
          ...
          <button 
            ...
            disabled={isScanning}
            className="flex flex-1 sm:flex-none justify-center items-center gap-2 bg-ff-primary text-white px-4 py-2 rounded-full font-bold text-sm hover:bg-ff-tertiary transition-colors disabled:opacity-50"
          >
      ...
      {/* Macro Overview */}
      <div className="flex flex-col md:flex-row gap-8 items-center justify-center p-8 rounded-[20px] bg-ff-surface border-2 border-ff-primary">
        <div className="relative w-48 h-48 flex items-center justify-center">
          <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
            <circle cx="50" cy="50" r="45" fill="none" stroke="var(--color-ff-empty)" strokeWidth="10" />
            <circle cx="50" cy="50" r="45" fill="none" stroke="var(--color-ff-primary)" strokeWidth="10" strokeDasharray="283" strokeDashoffset={Math.max(0, 283 - (totalCalories / 2400) * 283)} className="transition-all duration-1000" />
          </svg>
          <div className="absolute flex flex-col items-center justify-center text-center">
            <span className="text-3xl font-mono font-bold text-ff-text stat-number">{totalCalories}</span>
        ...
        <div className="flex md:flex-col gap-6 w-full md:w-auto overflow-x-auto pb-4 md:pb-0">
          {[
            { label: 'Protein', current: totalProtein, color: 'text-ff-primary', bg: 'bg-ff-primary', target: 180 },
            { label: 'Carbs', current: totalCarbs, color: 'text-ff-secondary', bg: 'bg-ff-secondary', target: 250 },
            { label: 'Fat', current: totalFat, color: 'text-ff-tertiary', bg: 'bg-ff-tertiary', target: 70 },
          ].map((macro) => (
            <div key={macro.label} className="min-w-[100px] flex flex-col items-center gap-2">
              <span className={cn("text-xl font-mono font-bold stat-number", macro.color)}>{macro.current}g</span>
              <span className="text-xs text-ff-muted font-bold">{macro.label}</span>
              <div className="h-1 w-full bg-ff-empty rounded-full overflow-hidden">
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          <div className="w-3 h-3 rounded-sm bg-ff-primary" />
        ...
        <div className="flex flex-wrap gap-2">
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} className={cn(
              "w-10 h-14 rounded-md transition-all cursor-pointer border-2",
              i < 7 ? "bg-ff-primary border-ff-primary shadow-[0_0_10px_rgba(0,119,182,0.4)]" : "bg-ff-empty border-ff-empty"
            )} />
          ))}
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-4">
          {meals.map((meal) => (
            <div key={meal.id} className="rounded-[20px] bg-ff-surface overflow-hidden border-2 border-ff-primary">
              <div className="p-4 flex justify-between items-center border-b-2 border-ff-primary">
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-sm bg-ff-primary" />
                  <span className="font-bold text-ff-text">{meal.type}</span>
                  <span className="text-xs text-ff-muted ml-2">{meal.time}</span>
                </div>
                <span className="font-mono font-bold text-ff-primary">{meal.calories} kcal</span>
              </div>
              <div className="p-4 space-y-3">
                <div className="flex justify-between text-sm">
                  <span className="text-ff-text font-medium">{meal.name}</span>
                  <div className="flex gap-3 text-xs font-mono">
                    <span className="text-ff-primary font-bold">{meal.protein}g P</span>
                    <span className="text-ff-secondary font-bold">{meal.carbs}g C</span>
                    <span className="text-ff-tertiary font-bold">{meal.fat}g F</span>
                  </div>
                </div>
              </div>
            </div>
...
function ManualMealModal({ onClose, onSave, initialData }: { onClose: () => void, onSave: (meal: any) => void, initialData?: any }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
      <div className="bg-ff-surface border-2 border-ff-primary rounded-2xl p-6 w-full max-w-md animate-in zoom-in-95 duration-200">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-display tracking-wide text-ff-text">LOG MEAL</h3>
          <button onClick={onClose} className="text-ff-muted hover:text-ff-text transition-colors">
            <X className="w-6 h-6" />
          </button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-bold text-ff-text mb-1">Meal Name</label>
            <input 
              required 
              type="text" 
              value={name} 
              onChange={e => setName(e.target.value)} 
              className="w-full bg-ff-bg border-2 border-ff-primary rounded-xl px-4 py-3 text-ff-text focus:outline-none focus:border-ff-secondary transition-colors" 
              placeholder="e.g., Chicken Salad" 
            />
          </div>
          <div>
            <label className="block text-sm font-bold text-ff-text mb-1">Type</label>
            <select 
              value={type} 
              onChange={e => setType(e.target.value)} 
              className="w-full bg-ff-bg border-2 border-ff-primary rounded-xl px-4 py-3 text-ff-text focus:outline-none focus:border-ff-secondary transition-colors appearance-none"
            >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-bold text-ff-text mb-1">Calories</label>
              <input 
                required 
                type="number" 
                min="0"
            ...
            <div>
              <label className="block text-sm font-bold text-ff-text mb-1">Protein (g)</label>
              <input 
                required 
                type="number" 
                min="0"
            ...
            <div>
              <label className="block text-sm font-bold text-ff-text mb-1">Carbs (g)</label>
              <input 
                required 
                type="number" 
                min="0"
            ...
            <div>
              <label className="block text-sm font-bold text-ff-text mb-1">Fat (g)</label>
              <input 
                required 
                type="number" 
                min="0"
          ...
          <div className="pt-2">
            <button 
              type="submit" 
              className="w-full bg-ff-primary text-white font-bold py-4 rounded-xl hover:bg-ff-tertiary transition-colors"
            >
              Save Meal
            </button>
          </div>
